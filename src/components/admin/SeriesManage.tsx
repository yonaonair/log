...
export default function SeriesManage() {
  ...
  const loadGroups = () => {
    fetch("/api/admin/posts")
      ...
      .then((posts: Post[]) => {
        ...
        setGroups([...map.entries()].sort((a,b) => a[0].localeCompare(b[0])).map(([name, posts]) => ({ name, posts })));
        setLoading(false);
      });
  };
  ...
  const handleRemoveSeries = async (seriesName: string) => {
            ...
            <tr key={g.name}>
                ...
                {renaming === g.name ? (
                  <div className="rename-row">
                    <input
                      ...
                      onKeyDown={e => { if (e.key === "Enter") handleRename(g.name); if (e.key === "Escape") setRenaming(null); }}
                      autoFocus
                    />
                    <button className="icon-btn-sm" onClick={() => handleRename(g.name)}><Check size={14} /></button>
              ...
              <td>
                <div style={{display:"flex",gap:4}}>
                  <button className="icon-btn-sm" title="이름 변경" onClick={() => { setRenaming(g.name); setRenameVal(g.name); }}>
                    <Pencil size={13} />
                  </button>
                  <button className="icon-btn-sm danger" title="시리즈 삭제 (글 유지)" onClick={() => handleRemoveSeries(g.name)}>
                    <X size={13} />
                  </button>
                </div>
              </td>
