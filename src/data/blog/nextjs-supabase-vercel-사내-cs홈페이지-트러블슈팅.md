...
modDatetime: 2026-05-15T01:16:08.450Z
...
`useMemo`는 컴포넌트가 실제로 마운트될 때 실행된다. 빌드 시 prerender 단계가 아니라 런타임에 실행되므로 환경변수가 정상적으로 주입된 상태다. `[]` 의존성 배열로 한 번만 생성하게 해서 불필요한 재생성도 막았다.
...
서버에서는 `window`가 없으니까 `typeof window !== "undefined"` 체크하면 될 거라고 생각한 코드다.

### 문제가 되는 이유 → `Hydration Mismatch`

Next.js `use client` 컴포넌트도 초기 렌더링은 서버에서 한 번, 클라이언트에서 한 번, 총 두 번 일어난다.
...
`useState` 초기값에서 `localStorage` 접근을 완전히 제거하고 `useEffect`로 마운트 이후에 복원한다.
...
`useEffect`는 서버에서 실행되지 않는다. 브라우저에 마운트된 이후에만 실행된다.

따라서 서버/클라이언트 초기 렌더 결과가 동일하게 빈값이 되고 `hydration`이 성공한다. 이후 `useEffect`가 실행되면서 `localStorage` 값이 복원되고 버튼이 활성화된다.
...
Next.js App Router에서 `layout.tsx`는 공통 UI(헤더, 내비게이션 등)를 담는 파일이다.
...
<div color="yellow" data-type="callout" data-color="yellow" class="callout callout-yellow">
<p>탭 클릭 → URL 변경 → layout.tsx 재실행 → Vercel 서버에서 Supabase로 쿼리 3번 → Supabase가 응답 → Vercel이 HTML 조립 → 브라우저에 전달</p>
</div>

이 왕복이 매 탭 클릭마다 일어났다. Vercel 서버와 Supabase가 지리적으로 다른 리전에 있을 경우 이 왕복만 수백 ms고, 쿼리 3번이니 직렬로 실행되면 2초에서 5초까지 걸렸다. 네트워크 탭에서 확인해보고 헛웃음이 나왔다. 당연히 사용에 불편을 느끼시고 개선을 요청하셨다.

### 해결 방법

### 1단계. 인증은 `middleware`로 이전

인증 체크(`supabase.auth.getUser()`)를 layout에서 꺼내 `middleware.ts`로 옮겼다.
...
middleware는 Next.js의 `Edge Runtime`에서 실행된다. Vercel의 일반 서버리스 함수보다 훨씬 가볍고 빠른데다 인증 쿠키 확인 정도는 충분히 처리 가능하다.
...
이제 layout은 HTML 구조만 담고 있다. 서버 쿼리가 없어서 바로 렌더링된다.
...
탭 클릭 후 지연 시간이 사용할 수 있는 수준이 되었다.

<div color="red" data-type="callout" data-color="red" class="callout callout-red">
<p>Next.js App Router에서 <code>layout.tsx</code>에 서버 쿼리를 넣으면 해당 레이아웃 아래 모든 라우트 이동마다 쿼리가 실행된다. 공통 레이아웃에서 데이터를 내려주는 패턴은 직관적으로 보이지만 페이지 이동이 잦은 앱에선 심각한 성능 문제가 된다.</p>
</div>
...
Next.js 서버 액션은 서버에서 실행되는 함수를 클라이언트에서 직접 호출하는 것처럼 쓸 수 있게 해주는 기능이다.
...
코드가 깔끔하고 타입도 맞아서 자연스럽게 선택했다. 근데 실제 실행 흐름을 따라가보면 문제가 보인다.
...
<div color="yellow" data-type="callout" data-color="yellow" class="callout callout-yellow">
<p>1. 브라우저에서 Vercel 서버로 HTTP POST 요청 전송 2. Vercel 서버: supabase.auth.getUser() 로 인증 확인 (Supabase 네트워크 요청 1회) 3. Vercel 서버: tasks 테이블 업데이트 쿼리 (Supabase 네트워크 요청 2회) 4. Vercel 서버가 브라우저로 응답 반환</p>
</div>
...
브라우저 환경에서는 `Supabase SDK`가 이미 로그인 시 세션을 `localStorage`에 저장해둔다. 클라이언트에서는 그걸 그냥 읽으면 된다. 굳이 서버에 물어볼 이유가 없다.
...
상태 변경이 클릭과 비슷하게 반영되는 수준이 됐다.

<div color="red" data-type="callout" data-color="red" class="callout callout-red">
<p>서버 액션은 서버에서만 가능한 작업(외부 API 키 사용, 파일 시스템 접근 등)에 써야 한다. 단순 DB 읽기/쓰기를 서버 액션으로 하는 건 Vercel 서버를 불필요한 중간 다리로 끼우는 것이다. Supabase RLS가 잘 설정돼 있다면 클라이언트에서 직접 호출하는게 더 빠르고 단순하다.</p>
</div>
